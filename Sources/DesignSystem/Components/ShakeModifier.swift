...
/// fields — pair with the `isError` flag on `DOBField` /
/// `GlassTextField` (or any field that exposes one).
///
...
/// DOBInputGroup(day: $d, month: $m, year: $y, isError: hasError)

...
/// Visual format for a `DSCountdownText`.
///
...
public enum DSCountdownStyle: Sendable {
    case compact
    case verbose
}
...
public struct DSCountdownText: View {
    private let targetDate: Date
    private let style: DSCountdownStyle

    public init(targetDate: Date, style: DSCountdownStyle = .compact) {
    ...
    public static func format(remaining seconds: TimeInterval, style: DSCountdownStyle) -> String {
...
#Preview("Countdown Text") {
    ...
    VStack(spacing: 16) {
        DSCountdownText(targetDate: target, style: .compact)
            .font(DSTypography.bodySmall)
            .foregroundStyle(DSColors.accentIndigo)

        DSCountdownText(targetDate: target, style: .verbose)
            .font(DSTypography.bodySmall)
